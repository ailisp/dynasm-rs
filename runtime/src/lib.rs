...
use crate::components::{LabelRegistry, ManagedRelocs, MemoryManager, PatchLoc, RelocRegistry};
...
impl<R: Relocation> DynasmLabelApi for VecAssembler<R> {
    ...
    fn backward_relocation(
        ...
        if loc.patch(buf, self.baseaddr, target).is_err() {
            self.error = Some(DynasmError::ImpossibleRelocation(TargetKind::Backward(
                name,
            )))
        }
    }
...
impl<R: Relocation> Assembler<R> {
    ...
    pub fn commit(&mut self) -> Result<(), DynasmError> {
        {
            let _span = tracing::debug_span!(target: "vm", "encode_relocs").entered();
            self.encode_relocs()?;
        }
        ...
        {
            let _span = tracing::debug_span!(target: "vm", "memory.commit").entered();
    ...
    pub fn finalize(mut self) -> Result<ExecutableBuffer, Self> {
        {
            // let _span = tracing::debug_span!(target: "vm", "dynasmrt Assembler finalize").entered();

            {
                let _span =
                    tracing::debug_span!(target: "vm", "dynasmrt Assembler commit").entered();
            ...
            // {
            //     let _span =
            //         tracing::debug_span!(target: "vm", "dynasmrt Assembler empty test").entered();
            // }
            {
                // let _span =
                //     tracing::debug_span!(target: "vm", "dynasmrt Assembler memory finalize")
                //         .entered();
...
impl<R: Relocation> DynasmLabelApi for Assembler<R> {
    ...
    fn local_label(&mut self, name: &'static str) {
        ...
        for loc in self.relocs.take_locals_named(name) {
            ...
            if loc
                .patch(buf, self.memory.execbuffer_addr(), offset.0)
                .is_err()
            {
                self.error = Some(DynasmError::ImpossibleRelocation(TargetKind::Forward(name)))
